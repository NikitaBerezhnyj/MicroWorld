...
local function buildButtons()
    ...
    buttons = {
        Button:new("New Game", cx - bw / 2, cy - bh / 2 - gap, bw, bh, function()
            ScreenManager.switch("game")
        end),
        Button:new("Load Game", cx - bw / 2, cy - bh / 2, bw, bh, function()
            ScreenManager.push("load", { mode = "load" })
        end),
        Button:new("Settings", cx - bw / 2, cy - bh / 2 + gap, bw, bh, function()
            ScreenManager.push("settings")
        end),
        Button:new("Exit", cx - bw / 2, cy - bh / 2 + gap * 2, bw, bh, love.event.quit),
    }
end
...
function MenuScreen:load() buildLayout() end

function MenuScreen:resize() buildLayout() end
...
function MenuScreen:draw()
    for _, c in ipairs(characters) do
        ...
        love.graphics.draw(img, c.x, c.y, math.rad(c.rotate),
            c.size / img:getWidth(), c.size / img:getHeight())
    end
    title:draw()
